	...
	public void saveOneRollBack(RollBack rollBack) {
		try {
			if(rollBack.getId() == 0) {
				...
				getJdbcTemplate().update("INSERT INTO HVC_BIRTHDAY_BONUS_ROLLBACK_EB (STEP,VALUE,ANUMBER,BNUMBER,ERROR_TIME,ERROR_TIME_INDEX) VALUES(" + rollBack.getStep() + "," + rollBack.getValue() + ",'" + rollBack.getAnumber() + "','" + rollBack.getBnumber() + "',TIMESTAMP '" + dateFormat.format(now) + "'," + Integer.parseInt(new SimpleDateFormat("yyyyMMdd").format(now)) + ")");			
			}
